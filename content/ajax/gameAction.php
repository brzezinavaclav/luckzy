...
if (empty($_GET['_unique']) || mysql_num_rows(mysql_query("SELECT `id` FROM `players` WHERE `hash`='".prot($_GET['_unique'])."' LIMIT 1 FOR UPDATE"))==0) exit();
...
switch ($_GET['action']) {
  ...
  case 'double':
    ...
    mysql_query("UPDATE `games` SET `bet_amount`=TRUNCATE(ROUND((`bet_amount`*2),9),8),`canhit`=0 WHERE `id`=$gameData[id] LIMIT 1");
  ...
  case 'hit':
    ...
    $deck[]=$newCard;
    mysql_query("UPDATE `games` SET `$whichDeck`='".serialize($deck)."',`used_cards`=(`used_cards`+1) WHERE `id`=$gameData[id] LIMIT 1");
...
$gData = mysql_fetch_array(mysql_query("SELECT * FROM `games` WHERE `id`=$gameData[id] LIMIT 1"));
